...
customer.file = (function () {
  ...
  // 
    ...
    var update_time;
        ...
        // 当前登录人姓名
        // $('tr.file .uname').val(configMap.user_info.uname);

        // 当前被点击的客户编号
        // $('tr.file .cnum').val(arg_map.this_line.find('td.cnum').text());
      },
  ...
  deleteFile = function (file_id) {
    $.ajax({
      ...
      success     : function (data) {
        if (data) {
          alert('成功删除文件！');
          customer.getCustomerListData('customer/zhuguan', {
            uname    : configMap.user_info.uname,
            position : configMap.user_info.position
          });
          setCustomerFileData();
          $('.extend').parent().find('.file').remove();
        } else {
          alert('删除客户文件失败!');
        }
      },
  ...
  // 
    ...
    jqueryMap.$pull.click(function () {
      ...
      $('.ora-user-customer-delete .confirm').unbind('click').click(function () {
        var that;
        that = stateMap.delete_file;
        console.log( that.parent().parent().parent().find('tr.extend').attr('file_id') );
        that.parent().parent().parent().find('tr.extend').attr('file_id', 0);
